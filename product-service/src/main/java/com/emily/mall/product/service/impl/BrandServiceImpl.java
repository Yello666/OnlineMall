...
public class BrandServiceImpl extends ServiceImpl<BrandMapper, Brand> implements BrandService {
     ...
     */
    @Override
    public Result<BrandVo> createBrand(Brand brand) {
        // 检查品牌名是否已存在
        ...
        return result > 0 ? Result.ok(vo) : Result.fail("创建品牌失败");
    }
     ...
     */
    @Override
    public Result<BrandVo> updateBrand(Brand brand) {
        if (brand.getId() == null) {
        ...
        BrandVo vo=new BrandVo(brand);
        return result > 0 ? Result.ok(vo) : Result.fail("修改品牌失败");
    }
}
